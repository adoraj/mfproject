...
//---------------处理返回信息------------
	...
	foreach($config['replaces'] as $replace){
		$replace['seach'] = get_magic_quotes_gpc()?stripslashes($replace['seach']):$replace['seach'];
		$seach = addcslashes(iconv("UTF-8",$charset,v($replace['seach'])),'/');
		$replace['replace'] = get_magic_quotes_gpc()?stripslashes($replace['replace']):$replace['replace'];
		$replace = iconv("UTF-8",$charset,v($replace['replace']));
